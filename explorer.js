...
(async function () {
  ...
  function preprocessData(raw) {
    return raw.map(d => ({
      ...d,
      _year: +d.year || null,
      _languages: (d.languages ?? []).map(l => l.trim()),
      _themes: (d.themes ?? []).map(t => t.trim()),
      _country: d.country?.trim() || 'Unknown',
      _publisher: d.publisher?.trim() || 'Unknown'
    }));
  }
  ...
  function drawTemporalTrends() {
    ...
    const years = [...processedData.years].filter(d=>d[0]).sort((a,b)=>a[0]-b[0]);
    const x = d3.scaleLinear().domain(d3.extent(years,d=>d[0])).range([0,w]);
    const y = d3.scaleLinear().domain([0,d3.max(years,d=>d[1])]).nice().range([h,0]);
    ...
    g.append('path').datum(years).attr('fill','none').attr('stroke','steelblue').attr('stroke-width',2).attr('d',line);
  ...
  function drawSankeyDiagram() {
    ...
    linksRaw.forEach(([k])=>{
      const [s,t] = k.split('|||');
      nodes.set(s,{name:s}); nodes.set(t,{name:t});
    });
    const links = linksRaw.map(([k,v])=>{const [s,t] = k.split('|||'); return {source:s,target:t,value:v};});

    const sankey = d3.sankey().nodeWidth(15).nodePadding(10).extent([[1,1],[w-1,h-5]]);
    ...
    g.append('g').selectAll('rect').data(graph.nodes).enter().append('rect')
      ...
      .attr('fill','steelblue').append('title').text(d=>`${d.name}: ${d.value}`);

    g.append('g').attr('fill','none').attr('stroke','#000').attr('stroke-opacity',0.2)
  ...
  function drawChordDiagram() {
    ...
    const svg = d3.select(el).append('svg').attr('width',size+margin.left+margin.right).attr('height',size+margin.top+margin.bottom)
      .append('g').attr('transform',`translate(${(size+margin.left+margin.right)/2},${(size+margin.top+margin.bottom)/2})`);

    svg.append('g').selectAll('g').data(chord.groups).enter().append('g')
      .append('path').attr('d',arc).attr('fill',d=>colour(d.index))
      .append('title').text(d=>`${langs[d.index]}: ${d.value}`);

    svg.append('g').attr('fill','none').attr('stroke-opacity',0.6)
      .selectAll('path').data(chord).enter().append('path')
      .attr('d',ribbon).attr('stroke',d=>colour(d.source.index)).attr('fill',d=>colour(d.source.index))
      .append('title').text(d=>`${langs[d.source.index]} → ${themes[d.target.index]}: ${d.source.value}`);
  }
  ...
  function drawParallelCoordinates() {
    ...
    // Build flat table [{year, langCount, themeCount}]
    ...
    // Background lines
    g.append('g').attr('stroke','#ccc').attr('stroke-width',1)
      .selectAll('path').data(data).enter().append('path').attr('d',path).attr('fill','none');

    // Axes
    dims.forEach(dim=>{
      g.append('g').attr('transform',`translate(${x(dim)},0)`).call(d3.axisLeft(yScales[dim]))
        .append('text').attr('y',-9).attr('text-anchor','middle').attr('font-weight',600).text(dim.toUpperCase());
    });
  }
